...
function App() {
  ...
  const handleAddClicked = productId => {
    ...
    const updatedItems =
        found ? items.map(v => (v.id === productId) ? {...v, count: v.count + 1} : v) :
            [...items, {...product, count: 1}]
    setItems(updatedItems);
    console.log(products.find(v => v.id === productId), "added");
  }
  useEffect(() => {
  ...
  return (
      <div className="container-fluid">
        <div className="row justify-content-center m-4">
          <h1 className="text-center">GNU Coffee Collection</h1>
        </div>
        <div className="card">
          <div className="row">
